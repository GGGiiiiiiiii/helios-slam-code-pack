...
> 说明：真机首选**原生里程计** `/sr_amr_control/odom`（v1.3.0，约 3.33Hz）+ **IMU** `/camera/gyro_accel/sample`（Orbbec，200Hz）经 EKF 融合（`use_ekf:=true`）；`odom_publisher` 速度积分作回退/仿真用。仍待实测：**雷达 z/对角外参、IMU 安装外参、速度上限**。
...
│   └── ekf.yaml              # EKF 融合原生 odom + Orbbec IMU（use_ekf:=true 启用）
├── launch/
...
| 项 | 命令 | 影响 |
|---|---|---|
| 原生 odom | `ros2 topic echo /sr_amr_control/odom --once` | 真机首选源（v1.3.0，3.33Hz）；喂 EKF |
| IMU | `ros2 topic info /camera/gyro_accel/sample -v` | 确认为 sensor_msgs/Imu（200Hz）；EKF 输入 |
| TF 树 | `ros2 run tf2_tools view_frames` | 需要 map→odom→base_footprint→base_link→laser |
| 雷达 frame | `ros2 topic echo /sr_amr_control/front/scan --once` | 决定 static_tf 的 child frame |

- 真机推荐 EKF 融合原生 odom + IMU（`use_ekf:=true use_odom:=false`）；无 EKF 时用 `odom_publisher` 速度积分（会漂移，靠激光修正）。
- 确认 **真实雷达外参 / IMU 安装外参** 后，修改 URDF 或 `launch/static_tf.launch.py` 里的 x/y/z/yaw/rpy。
