...
真机首选原生里程计：珞石 v1.3.0 已提供 /sr_amr_control/odom
（nav_msgs/Odometry，约 3.33Hz），建议直接喂 EKF（见 ekf.yaml），无需本节点积分。

本节点保留作两种用途：
  1. 原生 /sr_amr_control/odom 不可用时的回退（从 system_state 速度积分）。
  2. 不启用 EKF 时，直接产出 /odom + odom->base_footprint TF。

system_state 提供车体坐标系速度：
  linear_velocity_x / linear_velocity_y / angular_velocity
本节点对其积分，发布：
  - /odom (nav_msgs/Odometry)
  - TF: odom -> base_footprint（启用 EKF 时设 publish_tf=false，TF 由 EKF 发布）
