...
| `/odom` + `odom→base_footprint` | Gazebo planar_move | 原生 `/sr_amr_control/odom` + IMU 经 EKF（首选）；odom_publisher 积分作回退 |
...
| 文件 | 参数 | 说明 |
|---|---|---|
| `helios_description/urdf/helios.urdf.xacro` | `lidar_z` / 对角方向 | 雷达扫描面高度与对角安装（xy 已 STL 实测回填） |
| 同上 | `imu_link` 外参 | IMU 相对 base_link 安装位姿（Orbbec，待标定） |
| `helios_bringup/config/nav2_params.yaml` | `vx/vy/wz_max` | 速度上限（待厂家/实测；底盘尺寸/footprint 已回填） |

> 已回填真值：底盘尺寸（STL 0.68×0.64×0.408）、雷达 xy、原生 odom、IMU 数据源。
```
